...
| BL-RT-06 | Safe-boundary eligibility detection | Detect the first safe cognition handoff boundary only after the current DSH step fully commits; do not suppress continuation or switch lanes | Spike verified the real AgentLoop seam; unmerged PR #56 integrates committed `session/event` + actual `llm/stream` observation and a pinned DSH eligibility trace | VERIFIED_FACT_NOT_INTEGRATED | Runtime eligibility integration | A | INTEGRATION_TICKET |
