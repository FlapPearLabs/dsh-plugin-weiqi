# Companion Go — Ticket / Spike Graph V1.2

**Status:** REVIEW_PENDING
**Supersedes for review:** `TICKET_GRAPH_V1.1.md`
**Current Spec Authority:** `docs/spec/SPEC_LEAN_V0.1_R2.4.1_VERIFIED.md`
**Reason:**
- approved resign contract clarification (R2.4.1)
- exact §38 benchmark coverage
- Spec Phase A / B execution gate
- final acceptance semantics (F-T03 rescoped, F-T04 gate, F-T05 final)

**Base:** remote main `e82b376c147317dbae928a8a2bf2cd6be967dbbc`
**Branch:** `work/ticket-decomposition-v1`
**Parent HEAD:** `f13605aa56f2658c1c6024228d07859e084b133e` (V1.1)

**Method:** Verified Spec (R2.4.1) − Current Implementation Baseline = Remaining Gap Graph, per `docs/planning/TICKET_DECOMPOSITION_CONTRACT.md`. V1.2 is a fix of V1.1, not a re-decomposition.

**Delta vs V1.1:**
- Resign contract formalized in Spec R2.4.1 (§25/§26/§28/§31/§32/§36) and threaded through Baseline + Tickets (B-T01/B-T06/B-T05 11-case/E-T01/F-T01/F-T02).
...
- WAVE-C-T03 → Phase B (companion.* tools); WAVE-C-T01 re-scoped to Phase A core 7 go.* surface.
...
| A | 30 (A-T01..A-T07, B-T01..B-T06, C-T01, C-T02, C-T04..C-T10, D-T01..D-T04, E-T01, F-T01..F-T04) |
| B | 8 (C-T03, D-T05, D-T06, D-T07, D-T08, E-T02, F-T05) |
| Total | 44 |
...
### Wave B — GoRulesPort / Tenuki / rules fixtures (R2.4.1 resign)
...
- **Spec Authority:** §26 (R2.4.1): `createGame/play/pass/resign/getState/inspectGroup/score/settle`; GoRulesPort = sole application-facing game-mutation boundary; resign = terminal action at the port (no second rules authority); §30
...
- **Spec Authority:** §28 (R2.4.1): `CanonicalGameRecord`; actions play/pass/resign; replay marks terminal on resign without board mutation; winner/reason = opponent / resignation; no Tenuki internals; no second mutable result truth
...
- **Spec Authority:** §31 (R2.4.1): eleven cases — single capture, multi-stone capture, suicide rejection, simple ko transition, positional superko, pass, two-pass ending, area scoring, dead-stone settlement, canonical replay/restoration, **resignation terminal / canonical replay restoration**; §39.5
...
- **Current Baseline State:** BL-GT-01: NOT_IMPLEMENTED | **Target State:** IMPLEMENTED_VERIFIED (Phase A: core 7 go.* surface)
- **Spec Authority:** §18 (sole strategic policy owner), §20 (final V0.1 surface = 7 go.* + 2 companion.* = 9; Phase A proves the core 7 go.* are the only Go-strategy/action model-facing surface)
- **Scope (Phase A):** Core Go lane model-facing registry: prove the 7 `go.*` core tools are the only Go-strategy/action model-facing surface during Phase A; no solver/best-move/win-rate/general-execution surface. After WAVE-C-T03 (Phase B), the final V0.1 surface becomes 7 go.* + 2 companion.* = 9 — lifecycle/phase wording, no architecture change. C-T01 must NOT claim the complete 9-tool surface exists during Phase A.
- **Explicit Non-Goals:** No tool implementations (C-T02/C-T03); no preset isolation (C-T04).
- **Dependencies / blocked_by:** WAVE-C-T04
- **Expected Surfaces:** `src/tools/` registry, preset config
- **Acceptance Criteria:** Phase A registered Go-strategy/action surface = exactly the 7 `go.*` tools; smoke asserts no forbidden strategy/solver surface; no solver import; final 9-tool claim deferred to Phase B.
...
- **Spec Authority:** §20 (7 tools), §22.3 (accounting at actual execution), §32 (tool action is truth)
- **Scope:** Register/implement seven `go.*` tools over GoRulesPort; `go.try_move` returns legal/captures/local liberties only; wire budget accounting via C-T07 enforcement API.
- **Explicit Non-Goals:** No `companion.*` (C-T03, Phase B); no boost (C-T08); no strategy advice.
- **Dependencies / blocked_by:** WAVE-B-T02, WAVE-C-T01, WAVE-C-T06, WAVE-C-T07
- **Expected Surfaces:** `src/tools/go/`
- **Acceptance Criteria:** per-tool §20 shape (try_move no evaluative fields); illegal attempt no mutation; budget counter consumed.
- **Required Review Evidence:** tool tests; try_move schema; budget counter.
...
- **Spec Authority:** §32 (R2.4.1 core board), §33 (DSH-native shell; seats), §7.2 (board click → GoRulesPort directly), §34 (seam smoke)
...
- **Spec Authority:** §33/§35 (R2.4.1): Work View Go status / Go notice UX; Go View WorkSnapshot UX; clean mini-surface placement proven by E-S01; Companion-mode/persona peripheral presentation where applicable; latest-value projections only; no transcript fusion; no prototype rule/AI semantics
...
- **Spec Authority:** §36 (R2.4.1): restore sessions/canonical record/board/CompanionState/attention mode; default Work focus; no transactional recovery; **a resigned game remains terminal after restart/replay with same winner and reason = resignation**
...
- **Spec Authority:** §28/§36 (R2.4.1): replay play/pass through port; on resign mark terminal without board mutation; derive winner/reason; **resigned game remains terminal after replay**
...
- **Spec Authority:** R2.4.1 §40 complete (Correctness, Context Isolation, Continuity, Cognition Exclusivity, Anti-Cheat, Budget, Persona Continuity, Subjective UX)
...
- **Resign:** the approved contract is encoded in R2.4.1; any ticket requiring a GameLifecycleController / second game truth → `ESCALATION_REQUIRED`.
...
- `docs/spec/SPEC_LEAN_V0.1_R2.4.1_VERIFIED.md` (new — R2.4.1 from R2.4, resign clarification; R2.4 preserved)
...
- `AGENTS.md` (verified-Spec path → R2.4.1; Phase B execution rule)
- `PROJECT_STATUS.md` (Architecture: FROZEN — R2.4.1 VERIFIED; stage markers unchanged)
- `README.md` (Architecture marker → R2.4.1 VERIFIED)
