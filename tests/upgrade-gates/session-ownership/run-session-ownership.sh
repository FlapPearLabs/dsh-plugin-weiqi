...
fixture_root=$(mktemp -d "$dsh_root/packages/core/agent-loop/tests/companion-go-ownership.XXXXXX")
cleanup_fixture() {
  rm -rf "$fixture_root"
}
trap cleanup_fixture EXIT

cp "$script_dir/session-ownership.spec.ts" "$fixture_root/session-ownership.spec.ts"
cp -R "$plugin_root/src" "$fixture_root/plugin"

relative_spec=${fixture_root#"$dsh_root/"}/session-ownership.spec.ts
cd "$dsh_root"
