#!/usr/bin/env bash
# WAVE-D-S01 P2 negative + positive control for the fail-closed pin binding in
# run-system-prompt-context.sh.
#
# The mirror layout reproduces the CI gate shape: a "plugin repository root"
# holds package.json and the run script lives at
# <root>/tests/upgrade-gates/system-prompt-context/. The script under test is
...
run_script="$script_dir/run-system-prompt-context.sh"
...
mirror="$work/repo"
mkdir -p "$mirror/tests/upgrade-gates/system-prompt-context" "$work/dsh-stub"
...
run_gate() {
  ...
  (cd "$mirror" && DSH_PINNED_ROOT="$work/dsh-stub" bash "$mirror/tests/upgrade-gates/system-prompt-context/run-system-prompt-context.sh" >"$1" 2>"$2")
  echo $?
}

cp "$run_script" "$mirror/tests/upgrade-gates/system-prompt-context/run-system-prompt-context.sh"
...
if grep -q "PLUGIN_REPO_DSH_PIN_MISMATCH" "$work/pos.out" "$work/pos.err"; then
  echo "POSITIVE FAIL: pin mismatch reported on matching pins" >&2
  pos_fail=1
fi
...
echo "CONTROL-PIN-BINDING: ALL PASS"
