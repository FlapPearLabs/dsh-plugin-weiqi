...
fixture_root=$(mktemp -d "$dsh_root/packages/core/agent-loop/tests/companion-go-resume-sequencing.XXXXXX")
cleanup_fixture() {
  rm -rf "$fixture_root"
}
trap cleanup_fixture EXIT

cp "$script_dir/resume-sequencing.fixture.ts" "$fixture_root/resume-sequencing.spec.ts"
cp -R "$plugin_root/src" "$fixture_root/plugin"

relative_spec=${fixture_root#"$dsh_root/"}/resume-sequencing.spec.ts
cd "$dsh_root"
