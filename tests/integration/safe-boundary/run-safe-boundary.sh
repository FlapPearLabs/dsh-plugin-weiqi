...
fixture_root=$(mktemp -d "$dsh_root/packages/core/agent-loop/tests/companion-go-safe-boundary.XXXXXX")
cleanup_fixture() {
  rm -rf "$fixture_root"
}
trap cleanup_fixture EXIT

cp "$script_dir/safe-boundary.fixture.ts" "$fixture_root/safe-boundary.spec.ts"
cp -R "$plugin_root/src" "$fixture_root/plugin"

relative_spec=${fixture_root#"$dsh_root/"}/safe-boundary.spec.ts
cd "$dsh_root"
